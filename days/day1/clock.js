...
/**
 * 声明几个数组：monthText、dayText、weekText、hourText 、minuteText、secondsText， 
 * 分别各自文本，对应月、日、星期、时、分、秒。主要是为了方便一会生成DOM
 */
...
/**
 * 为了操作DOM，将DOM分类存放。如月份相关的DOM放在monthList中，星期相关的DOM就放在weekList，依次类推
 */
var monthList = [];
var dayList = [];
var weekList = [];
...
/**
 * 定义一个初始化函数init，该函数主要用于生成标签元素，并将对应的文本填入同时添加class为'label'，
 * 然后将生成的标签存放在各自的list中，最后在页面加载后调用init。
 */
function init() {
...
/**
 * 这里还用到自定义函数initStyle,该函数主要作用是将所有label标签初始为灰色，如下
 */
function initStyle() {
